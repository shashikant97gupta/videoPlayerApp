...
$(document).ready(function() {
    function createPlaylistCard(obj, pos) {
        ...
        mainDiv.setAttribute("videoId", obj.id);
    ...
    function addCurrentVideo(id){

        for(j=0; j< videoPlaySectionData.length; j++){
            document.documentElement.scrollTop = 0;
            document.scrollTop = 0;
            if(videoPlaySectionData[j].id == id  ){
                $("#video-player").attr("src", "https://player.vimeo.com/video/"+videoPlaySectionData[j].vimeoId);
                $("#views-count").html(videoPlaySectionData[j].views);
                $("#video-title").html(videoPlaySectionData[j].title);
                $("#video-description").html(videoPlaySectionData[j].description);
                if(videoPlaySectionData[j].isLiked == true){
                    $(".far fa-heart").removeClass("far fa-heart").addClass("fas fa-heart").css({
                        color : "red",
                    });
                };
                if(videoPlaySectionData[j].isSaved == true){
                    $(".far fa-bookmark").removeClass("far fa-bookmark").addClass("fas fa-bookmark").css("color", "#ccc");
                };
    ...
    $(".playlist-card").on("click", function(){
        ...
        var videoId = this.attributes.videoid.nodeValue;
        // console.log(this.attributes.videoid.nodeValue);
        addCurrentVideo(videoId);
    });
